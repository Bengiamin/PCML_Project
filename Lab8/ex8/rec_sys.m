...
for i = 1:N
    index = find(X(:, i) ~= 0);
    user_mean(i) = mean(X(index, i));
    index2 = find(Xtest(:, i) ~= 0);
    diff = [diff; Xtest(index2, i) - user_mean(i)];
end

rmse_um = sqrt( mean(diff .^ 2)  );
...
% INSERT CODE
movie_mean = zeros(D, 1);

for i = 1:D
    index = find(X(i, :) ~= 0);
    movie_mean(i) = mean(X(i, index));
    index2 = find(Xtest(i, :) ~= 0);
    diff = [diff; (Xtest(i, index2) - movie_mean(i))'];
end

rmse_mm = sqrt( mean(diff .^ 2)  );
